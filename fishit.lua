...
local function charge()
    ...
    task.wait(0.1)
    select(1, safeInvoke(RFChargeFishingRod, 2))
    local x, z = getDirXZ()
    select(1, safeInvoke(RFStartMinigame, x, z))
end

local gotFish = false

-- Saat server kirim sinyal "FishCaught"
REFishCaught.OnClientEvent:Connect(function(payload)
	if not running then return end
...
-- Saat teks efek muncul (minigame mulai)
...
local function buildUI()
    ...
    -- Tombol tambahan: Repair
    local repairBtn = Instance.new("TextButton")
    repairBtn.Text = "Repair"
    repairBtn.TextScaled = true
    repairBtn.Font = Enum.Font.GothamBold
    repairBtn.Size = UDim2.fromScale(1, 0.18)
    repairBtn.BackgroundColor3 = Color3.fromRGB(55,55,55)
    repairBtn.TextColor3 = Color3.fromRGB(255,255,255)
    repairBtn.Parent = pgAuto
    ...
    repairBtn.MouseButton1Click:Connect(function()
        if repair then
            ...
            warn("Function 'repair' belum didefinisikan.")
        end
    end)
